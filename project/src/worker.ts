...
// 生成缓存键
type Env = {
  BIOME_STATE: any; // DurableObjectNamespace - 由 Cloudflare Runtime 提供
  AI: any;
  DB: any;
  R2: any;
  VECTOR_INDEX: any;
  ...
  async fetch(request: Request, env: Env) {
    ...
    if (url.pathname === '/api/mutate') {
      try {
        ...
        if (isGM) {
          ...
          const req: MutationRequest = { targetPart, intent, currentGenome, environmentContext };
          const parseRaw = await env.AI.run('@cf/meta/llama-3.1-8b-instruct', {
    ...
    if (url.pathname === '/api/evolution-tree') {
      try {
        ...
        const topN = 50;
        const results = await env.VECTOR_INDEX.query([0.1, 0.2], { // 哑查询以获取最近项
          topK: topN,
          returnMetadata: true
        });
