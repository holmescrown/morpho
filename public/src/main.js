...
function animate() {
    ...
    if (window.CALAMITY_STATE.isDead) {
        ...
        if (window.currentGenomeData) {
            ...
            // 死亡清算：能量枯竭且尚未走入死亡流程
            if (window.currentGenomeData.energy_reserve <= 0 && !window.CALAMITY_STATE.isDead) {
                if (window.debugForceDeath) window.debugForceDeath();
            }
            // 桌面挂机预警：Hunger (< 10%) 致命衰竭心跳闪烁
            else if (window.currentGenomeData.energy_reserve < 10 && archetypeBuilder.organismState.coreMeshRef && !window.CALAMITY_STATE.isDead) {
                ...
                // 听觉脉冲联动
                if (window.heartbeatGain && typeof audioCtx !== 'undefined' && audioCtx && audioCtx.state === 'running') {
    ...
    // --- 开发者调试与自然法庭接口 (Developer / God Tools) ---
    window.CALAMITY_STATE = window.CALAMITY_STATE || { active: false, isDead: false, intensity: 0 };

    window.debugTriggerAcidification = () => {
        if (window.CALAMITY_STATE.isDead) return;
        window.updateSensoryLog("⚠️ [DEV] 强制环境干涉：大酸化灾变已触发！", "#ff3333");
        window.CALAMITY_STATE = {
            active: true,
            type: 'ACIDIFICATION',
        ...
        // 加速扣除 ATP 引起恐慌动画
        if (window.currentGenomeData) {
            window.currentGenomeData.energy_reserve = Math.max(0, window.currentGenomeData.energy_reserve - 50);
        }
    };
    ...
    window.debugForceDeath = () => {
        if (window.CALAMITY_STATE.isDead) return;
        window.CALAMITY_STATE.isDead = true;
        if (window.currentGenomeData) window.currentGenomeData.energy_reserve = 0;

        // 90/10 命运结算引擎 (Roguelike Death Roll)
        const roll = Math.random();
        console.log(`[Death Engine] RNG Roll: ${roll.toFixed(3)}`);

        if (roll <= 0.90) {
            window.updateSensoryLog("💀 [DEV] 90% 命运：灰飞烟灭。序列已溶解归环。", "#555555");
            // 粒子消散特效简替：快速缩小
            gsap.to(fishGroup.scale, {
                x: 0.01, y: 0.01, z: 0.01, duration: 2.0, ease: "power2.in", onComplete: () => {
                    scene.remove(fishGroup);
                    setTimeout(() => location.reload(), 3000); // 3秒后重启轮回
                }
            });
        } else {
            window.updateSensoryLog("🔶 [DEV] 10% 奇迹：跨越死亡。固化为金缮实体。", "#ffcc00");
            window.APP_MODE = 'DEATH_PROCESSING'; // 切入遗产着色器逻辑
            // 停止活动
            isPlayerMoving = false;
        }
    };

    window.debugToggleFPS = () => {
        window.__LOCK_30FPS = !window.__LOCK_30FPS;
        window.updateSensoryLog(
            window.__LOCK_30FPS ? "⚡ [DEV] FPS 节流：已锁定 30 帧 (Tauri/省电)" : "⚡ [DEV] FPS 释放：性能最大化",
            "#00f2ff"
        );
    };
